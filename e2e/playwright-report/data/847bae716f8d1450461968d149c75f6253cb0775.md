...
Call log:
  - waiting for getByRole('textbox', { name: 'Ingresa tu correo' })

    at C:\Users\jmanu\OneDrive\Escritorio\backUp Jose Manuel\Universidad\Tesis\tests\e2e\tests\evaluator.spec.ts:6:68
```
...
>  6 |     await page.getByRole('textbox', { name: 'Ingresa tu correo' }).click();
  ...
  106 |     await page.getByRole('menuitem', { name: 'Cerrar sesión' }).click();
```